...
dmt <- read.csv("./cleaneddata/lucid.csv")
head(dmt)
``` 
```{r}
summary(dmt)
...
coeff_treat_rnd <- function(d) {
  ...
  lmr = lm(trust ~ treat_rnd, data=dmt, na.action = na.omit)
...
```{r}
# 1K Randomizations
h.distribution.under.sharp.null <- replicate(10000, coeff_treat_rnd(dmt))
