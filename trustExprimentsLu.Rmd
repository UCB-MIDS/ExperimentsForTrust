...
``` 
```{r}
dlu$Outcome <- ifelse(dlu$Q7=="I do not trust what the speaker was saying.", "Trust", ifelse(dlu$Q7=="I trust what the speaker was saying.", "No Trust", "NA"))
...
coeff_treat_rnd <- function(d) {
  ...
  lmr = lm(trust ~ treat_rnd, data=dlu, na.action = na.omit)
...
```{r}
# p value
pv = mean(h.distribution.under.sharp.null >= ate)
paste0('p value: ', pv)
```
```{r}
# num of assignments that generate an estimated ATE at least as large as the actual
n <- sum(h.distribution.under.sharp.null >= ate)
